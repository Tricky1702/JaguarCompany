...
uniform sampler2D   uEffectsTexture;
uniform float       uSpecExponent;
uniform float       uTime;
uniform bool        uLightsActive;

float glow(float value, float timeScale)
{
    float t = cos(uTime * timeScale);

    return ((t + 1.0) / 2.0) * value;
}
...
void main()
{
    ...
    vec3 effectsMaterial = texture2D(uEffectsTexture, vTexCoord).rgb;
    ...
    if (uLightsActive)
        color += effectsMaterial * glow(0.5, 2.0);
