...
(function () {
    ...
    /* OXPConfig2 settings. */
    this.oxpcLookup = function oxpcLookup() {
        this.oxpcSettings = {
            Info : {
                Name : this.name,
    ...
    this.$uniqueShipName = function $uniqueShipName() {
        if (this.$availableShipNames.length === 0) {
            this.$availableShipNames = this.$jaguarCompanyShipNames.concat([]);
        }

        return "Jaguar Company: " + this.$availableShipNames.splice(Math.floor(Math.random() * this.$availableShipNames.length), 1).pop();
    };

    this.$spawnJaguarCompany = function $spawnJaguarCompany(state) {
        var basePosition,
        base,
        clutter;
        ...
        /* Initialise the number of ships to the maximum number. */
        this.$numShips = this.$maxShips;

        if (system.isInterstellarSpace) {
            /* If we are in interstellar space then the base is somewhere within 1.5 times scanner range of the centre point. */
            basePosition = Vector3D.random(38400.0);
        } else {
            /* Place the base 0.1 to 0.2 units along the witchpoint -> sun route. */
            basePosition = Vector3D.interpolate([0, 0, 0], system.sun.position, 0.1 + (Math.random() * 0.1));
        }
        ...
        /* Add some clutter within 15km around the base. 6 to 17 items. */
        clutter = system.addShips("asteroid", Math.floor(Math.random() * 6) + 6, basePosition, 15000);

        /* Add Jaguar Company within 5km of the base. */
        system.addShips("jaguar_company_patrol", this.$maxShips, basePosition, 5000.0);
    };

    this.$setUpCompany = function $setUpCompany() {
