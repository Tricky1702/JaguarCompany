...
(function () {
    ...
    this.startUp = function () {
        ...
        this.systemName = "";
        this.shipGroup = new ShipGroup("Jaguar Company");
    };
    ...
    this.$spawnJaguarCompany = function (state) {
        ...
        if (system.isInterstellarSpace) {
            position = Vector3D.random(38400.0);
        } else {
            position = Vector3D.interpolate([0, 0, 0], system.sun.position, 0.1 + (Math.random() * 0.1));
        }

        position = position.add(Vector3D.randomDirection(51200.0));
        base = system.addShips("jaguar_company_base", 1, position);
        ships = system.addShips("jaguar_company_patrol", 4, position, 10000.0);

        this.shipGroup.leader = ships[0];
        this.shipGroup.addShip(ships[1]);
        this.shipGroup.addShip(ships[2]);
        this.shipGroup.addShip(ships[3]);

        if (this.logging && this.logExtra) {
            log(this.name, "pos: " + position);
            log(this.name, "base: " + base);
            log(this.name, "SG: " + this.shipGroup);

            for (i = 0; i < this.shipGroup.ships.length; i++) {
                log(this.name, "ship #" + i + ": " + this.shipGroup.ships[i]);
            }
        }
    };
